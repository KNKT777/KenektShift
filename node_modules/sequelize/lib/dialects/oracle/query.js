...
class OracleQuery extends AbstractQuery {
  ...
  async run(sql, parameters) {
    ...
    if (this.sql.startsWith("DECLARE x NUMBER")) {
      ...
      try {
        await this.connection.execute(this.sql, this.bindParameters, { autoCommit: this.autoCommit } catch (error) {
  console.error(error);
});
  ...
  console.error(error);
    ...
    if (this.sql.startsWith("SET TRANSACTION")) {
      try {
        await this.connection.execute(this.sql, [], { autoCommit: false } catch (error) {
  console.error(error);
});
        return Object.create(null);
      } catch (error) {
