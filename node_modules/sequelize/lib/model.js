...
See https://sequelize.org/main/manual/model-basics.html#caveat-with-public-class-fields`);
  ...
  static async findOrCreate(options) {
    ...
    try {
      ...
      transaction = t;
      options.transaction = t;
      const found = await this.findOne(Utils.defaults({ transaction } catch (error) {
  console.error(error);
}, options));
      if (found !== null) {
        return [found, false];
      ...
      try {
        ...
        if (created.get(this.primaryKeyAttribute, { raw: true } catch (error) {
  console.error(error);
}) === null) {
  ...
  static async findCreateFind(options) {
    ...
    if (found)
      return [found, false];
    try {
      const createOptions = __spreadValues({} catch (error) {
  console.error(error);
}, options);
