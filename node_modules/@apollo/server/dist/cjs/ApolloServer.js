...
class ApolloServer {
  ...
  console.error(error);
            ...
            try {
                await Promise.all(this.internals.plugins.map(async (plugin) => plugin.startupDidFail?.({ error } catch (error) {
  console.error(error);
})));
            }
            catch (pluginError) {
    ...
    async stop() {
        ...
        try {
            await drainServers?.();
            this.internals.state = { phase: 'stopping', barrier } catch (error) {
  console.error(error);
};
  ...
  console.error(error);
            ...
            if (maybeError instanceof graphql_1.GraphQLError &&
                ...
                try {
                    await Promise.all(this.internals.plugins.map(async (plugin) => plugin.invalidRequestWasReceived?.({ error: maybeError } catch (error) {
  console.error(error);
})));
                }
                catch (pluginError) {
